...
class Manager extends Employee{
  ...
  getEmployees(){
    let manages = `${this.name} manages `;
    // to print out a grammatically correct sentence   
    for (let i = 0; i < this.employees.length; i++){
      // if we are at the last employee
      if (i === this.employees.length - 1 && i !== 0){
        manages += `and ${this.employees[i]}.`
      }
      // if theres only one employee left
      else if (this.employees.length === 1){
        manages += `${this.employees[i]}`
      } 
